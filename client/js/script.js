...
/* ══════════════════════════════════════
   DEEP SPACE STARFIELD — Dense tiny white pinpoints
   revolving diagonally. Clean, minimal, realistic.
   Canvas is position:fixed via CSS.
   ══════════════════════════════════════ */
(function () {
    ...
    /* ── Diagonal orbit direction (~30° from horizontal, drifts right-and-up) ── */
    const ORBIT_ANGLE = Math.PI / 5.5; // ~33 degrees
    const COS_A = Math.cos(ORBIT_ANGLE);
    const SIN_A = Math.sin(ORBIT_ANGLE);

    /* ── 3 depth layers — all tiny, lots of them ── */
    const LAYERS = [
        { name: 'far',   count: 6000, sizeMin: 0.2,  sizeMax: 0.5,  alphaMin: 0.08, alphaMax: 0.25, speedFactor: 0.15 },
        { name: 'mid',   count: 2500, sizeMin: 0.4,  sizeMax: 0.8,  alphaMin: 0.15, alphaMax: 0.45, speedFactor: 0.35 },
        { name: 'near',  count: 600,  sizeMin: 0.7,  sizeMax: 1.3,  alphaMin: 0.30, alphaMax: 0.75, speedFactor: 0.65 },
    ];
    ...
    function createStar(cfg) {
        ...
        const speed = cfg.speedFactor * (0.7 + Math.random() * 0.6);
        ...
        return {
            ...
            size,
            baseAlpha,
            // Diagonal velocity — moves right & up
            vx: speed * COS_A,
            vy: -speed * SIN_A,
    ...
    /* ── Wrap star around edges ── */
    function wrapStar(s, w, h) {
        const buf = 30;
        if (s.x > w + buf)  { s.x = -buf; s.y = Math.random() * h; }
        if (s.x < -buf)     { s.x = w + buf; s.y = Math.random() * h; }
        if (s.y < -buf)     { s.y = h + buf; s.x = Math.random() * w; }
        if (s.y > h + buf)  { s.y = -buf; s.x = Math.random() * w; }
    }
    ...
    function draw() {
        ...
        for (const layer of layers) {
            for (const s of layer.stars) {
                ...
                const a = Math.max(0, Math.min(1, s.baseAlpha + pulse * s.baseAlpha * 0.3));
    ...
    window.addEventListener('resize', () => {
        ...
        resizeTimer = setTimeout(() => {
            ...
            for (const layer of layers) {
                for (const s of layer.stars) {
                    if (s.x > canvas.width + 100 || s.y > canvas.height + 100 || s.x < -100 || s.y < -100) {
